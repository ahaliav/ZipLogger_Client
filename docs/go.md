...
A standard-library-only client with a first-class `log/slog` handler. Batching, retry with
backoff (429-aware), drop-on-backpressure, automatic enrichment. Go ≥ 1.21.
...
as NDJSON batches (default 100 per request, 2 s linger) to `/ingest/v1/logs`, retry transient
